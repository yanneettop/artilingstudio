...
if (grid) {
  ...
  if (filtersMount) filtersMount.innerHTML = filters.filter(([, value]) => value === "all" || materials.some((material) => material.filterGroups.includes(value))).map(([label, value], index) => `<button class="tile-style-filter${index === 0 ? " is-active" : ""}" type="button" data-tile-style-filter="${value}" aria-pressed="${index === 0}">${label}</button>`).join("");
